...
class Dataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        with h5py.File(self.item_paths[index], "r") as f:
            ...
            img_name = np.array(f.get("img_number"))
            return nerf_embedding, clip_embedding, data_dir, img_name, class_id


def group_dataset():
    dset_root = Path("controlnet")
...
def write_batch_to_file(batch, idx, split):
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        img_numbers=img_numbers.detach().cpu().numpy()
